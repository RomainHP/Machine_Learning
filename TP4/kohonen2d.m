...
function [w] = kohonen2d(x,K,mu,sigma,nbiter)
  ...
  for i = 1:nbiter
    ...
    q = ones(1,2);  ## index of winning component
    for j = 1:K
      for k = 1:K
        #d = (x(:,individuAuHasard)-[w(j,k,1) w(j,k,2)])' * (x(:,individuAuHasard)-[w(j,k,1) w(j,k,2)]);
        d = norm(x(:,individuAuHasard)-[w(j,k,1) w(j,k,2)]);
        #d = (x(:,individuAuHasard)-w(j,k,:))' * m * (x(:,individuAuHasard)-w(j,k,:));
        if (j==1 && k==1)
          dmin = d;
        else
          if (d < dmin)
